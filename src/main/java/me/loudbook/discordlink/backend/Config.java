...
public class Config {
    ...
    public void validateConfig(){
        if (this.properties.getProperty("discord-token") == null) {
            ...
            System.exit(1);
        } else if (this.properties.getProperty("msa-token") == null) {
            System.out.println("[ERROR] msa-token is not set in config.properties");
            System.exit(1);
        } else if (this.properties.getProperty("main-channel-id") == null) {
            System.out.println("[ERROR] main-channel is not set in config.properties");
            System.exit(1);
        } else if (this.properties.getProperty("officer-channel-id") == null) {
            System.out.println("[ERROR] officer-channel is not set in config.properties");
            System.exit(1);
        } else if (this.properties.getProperty("use-webhook") == null) {
            System.out.println("[ERROR] use-webhook is not set in config.properties");
            System.exit(1);
        } else if (Boolean.parseBoolean(this.properties.getProperty("use-webhook")) && this.properties.getProperty("guild-webhook-url") == null) {
            if (this.properties.getProperty("guild-webhook-url") == null) {
                System.out.println("[ERROR] guild-webhook-url is not set in and use-webhook is true.");
                System.exit(1);
            }
        } else if (Boolean.parseBoolean(this.properties.getProperty("use-webhook")) && this.properties.getProperty("officer-webhook-url") == null) {
            System.out.println("[ERROR] officer-webhook-url is not set in and use-webhook is true.");
            System.exit(1);
